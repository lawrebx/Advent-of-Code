...
The monkeys take turns inspecting and throwing items. On a single monkey's turn, it inspects and throws all of the items it 
is holding one at a time and in the order listed. Monkey 0 goes first, then monkey 1, and so on until each monkey has had one turn. 
...
Monkey 2: 
Monkey 3: 
Chasing all of the monkeys at once is impossible; you're going to have to focus on the two most active 
monkeys if you want any hope of getting your stuff back. Count the total number of times each monkey inspects items over 20 rounds:
...
class monkey():
    def __init__(self, id, operation, op_value, divisor, true_monke, false_monke,starting_items=list):
        self.id = id
        ...
        self.new_items = starting_items
        self.inspection_count = 0

    def catch_item(self, item):
        self.new_items.append(item)

    def inspect_items(self,part=1,mod_all=0):

        action_list = []

        item_list = self.new_items.copy()
        ...
        for item in item_list:
            ...
            if self.operation == '*':
                new_item = self.new_items.pop(0) * op_value
            elif self.operation == '+':
                new_item = self.new_items.pop(0) + op_value
            else:
            ...
            toss_monke = self.false_monke

            if part == 1:
                new_item = int(new_item/3)

            else:
                new_item = new_item % mod_all
                if new_item % self.divisor == 0:
                    toss_monke = self.true_monke           

            action = [toss_monke, new_item]
...
monkey_list = [monkey(0,'*',17,2,1,6,[83,62,93]),
               monkey(1,'+',1,17,6,3,[90,55]),
               monkey(2,'+',3,19,7,5,[91, 78, 80, 97, 79, 88]),
               monkey(3,'+',5,3,7,2,[64, 80, 83, 89, 59]),
               monkey(4,'*',0,5,0,1,[98, 92, 99, 51]),
               monkey(5,'+',2,13,4,0,[68, 57, 95, 85, 98, 75, 98, 75]),
               monkey(6,'+',4,7,3,2,[74]),
               monkey(7,'*',19,11,4,5,[68, 64, 60, 68, 87, 80, 82])
              ]        
...
for round in range(rounds):
    ...
    for monke in monkey_list:
        #print(monke.id)
        action_list = monke.inspect_items(2,mod_all)
        for action in action_list:
            monkey_list[action[0]].catch_item(action[1])
    

for monke in monkey_list:
    print(f'{monke.id} inspected {monke.inspection_count} items by the end of {rounds} rounds.')
    monkey_business.append(monke.inspection_count)

monkey_business.sort(reverse=True)

print(monkey_business)

print(f'Total monkey business: {monkey_business[0]*monkey_business[1]}')