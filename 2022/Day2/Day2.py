...
with open(input_file) as f:
    input = [x.split() for x in [line.rstrip() for line in f]]
...
def round_result(round_input,game_rules=dict):
    # determine win, return score or 0
    choice = game_rules[round_input[1]][1]
    if round_input[0] == game_rules[round_input[1]][0]:
        return choice + 6
    elif round_input[0] == game_rules[round_input[1]][2]:
        return choice + 3
    else:
        return choice

total_score = sum([round_result(x,rules_dict) for x in input])
...
# {target move : [win move, draw move, lose move,target score]}
rules_dict = {'A':['C','A','B',1], #rock
              'B':['A','B','C',2], #paper
              'C':['B','C','A',3]  #scissors
             }

# {strategy move : [move selection, outcome score]}             
strat_dict = {'X':[0,0], #lose
              'Y':[1,3], #draw
              'Z':[2,6]  #win
              }

def strat_result(round_input,game_rules=dict,strat=dict):
    move = strat[round_input[1]]
    return move[1] + game_rules[game_rules[round_input[0]][move[0]]][3]

total_score = sum([strat_result(x,rules_dict,strat_dict) for x in input])
