...
#def prioritize_rucksack(rucksack):
...
def get_common_items(rucksack):
    ruck_split = int(len(rucksack)/2)
    first_compartment = [*rucksack[0:ruck_split]]
    second_compartment = [*rucksack[ruck_split:len(rucksack)]]
    priority_sum = sum([char_position(item) for item in list(set(first_compartment) & set(second_compartment))])
    return priority_sum

common_item = sum([get_common_items(x) for x in input])
...
Additionally, nobody wrote down which item type corresponds to each group's badges. The only way to tell which item type is the right one is by finding the 
...
def get_badge(full_input, grp_size=3):
    
    groups = 0

    group_badges = []

    while len(full_input) > 0:
        ...
        for i in range(grp_size):
            grp_ct = grp_ct + 1
            poplist = set(list(full_input.pop()))
            if grp_ct == 1:
                badge_set.update(poplist)
            else:
                badge_set = badge_set.intersection(poplist)

        print(badge_set)
        
        group_badges.extend(list(badge_set))

    badge_sum = sum([char_position(x) for x in group_badges])

    return badge_sum

print(get_badge(input, grp_size=3))
