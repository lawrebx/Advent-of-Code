...
screen and simple CPU that are both driven by a precise clock circuit. The clock circuit ticks at a constant rate; each tick 
is called a cycle.
...
for step in steps:
    step_cycles = cycle_dict[step[0]]
    #print(step)
    #print(step_cycles)
    for cycle in range(step_cycles):
        cycle_count = cycle_count + 1
        #print(f'Cycle #:{cycle_count}')
        #print(f'Cycle Start Value: {cycle_value}')
        if cycle_count in cycle_check:
            cycle_obs.append(cycle_value*cycle_count)
        if step[0] == 'addx' and cycle == step_cycles-1:
            cycle_value = cycle_value + int(step[1])
        #print(f'Cycle End Value: {cycle_value}')
...
three pixels is the pixel currently being drawn, the screen produces a lit pixel (#); otherwise, the screen leaves the pixel dark (.).
...
cycle_count = 0
cycle_obs = []
cycle_check = [40,80,120,160,200,240]
rows = 6
row_pos = 0
...
for step in steps:
    ...
    for cycle in range(step_cycles):
        ...
        sprite_pos = [cycle_value-1,cycle_value,cycle_value+1]

        #print(f'Cycle #:{cycle_count}')
        #print(f'Cycle Start Value: {cycle_value}')
        #print(f'Row position: {row_pos}, Sprite position {sprite_pos}')      

        if row_pos in sprite_pos:
            cycle_obs.append('#')
            #print('#')
        else:
            cycle_obs.append('.')
